...
The use of \ac{MDM} software does not guarantee compliance;
  one survey found over 50\% of companies with \ac{MDM} software still have devices that do not comply with their policies in their networks~\cite{mobileiron_security_labs_q4_2015}.
...
When considering what tools a company may use Morrow notes that \emph{``particularly with the BYOD trend IT professionals don't know if anti-virus software is installed or if it it's current''}~\cite{morrow_byod_2012}.
...
Their work shows how the app-specific parts of a BYOD policy can be checked for and enforced using tools but they do not look at where the checks or polices come from.
...
We can delegate to different marketplaces (assuming they provided signed \AppPAL{} statements) to decide if an app meets different aspects of a policy, and create new ones combining them without having to rerun any checks they performed.
...
This software supports app wrapping, and supports security ``checkboxes'' to enable security controls and access control policies, geofencing could be used to apply additional policies in the area around a lab.
...
To answer these questions we present \AppPAL{}~as a tool for linking policies to the tools we use to implement them and distributing decisions about apps.
...
\todo{Add common policy idioms such as device configuration, restrictions, and the like to the table}
...
In general delegations are used when a policy decision requires input from a third-party.
...
It is interesting to compare \AppPAL~to existing MDM~tools such as IBM's MaaS360 and BlackBerry's Enterprise Mobility Management, both leading MDM packages~\cite{rob_smith_magic_2016}.
...
Our work has instead looked at the policies at a higher level, tracking, managing and authorizing policies based on what people have said and what tools were run.
...
Future work will also work on tying down the distribution mechanisms and defining the distribution protocol and how it should integrate with the public keys signing the assertions.

\todo{Nice ending paragraph.}
\todo{Sort the bibliography.}
\bibliography{paper}{}
