...
Because \AppPAL's predicates have a predictible naming conventions (\autoref{ssec:changes}), our tooling can also generate additional rules to check if the acknowledgement has been made, and check whether there is anyone to acknowledge the policy. 
\begin{lstlisting}
'company' says E hasSatisfiedObligationToMustAcknowledged('policy')
  if E mustAcknowledged('policy').
     E hasAcknowledged('policy').
\end{lstlisting}
