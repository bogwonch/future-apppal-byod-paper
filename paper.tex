...
The use of \ac{MDM} software does not guarantee compliance.
...
The policies are becoming more complex and \ac{MDM} software is becoming more proficient.
...
An antivirus or \ac{MDM} program may be used to \emph{implement} part of a policy.
It is the policy, however, that \emph{specifies} which software to use and when. 
There is no automatic way to check how a policy has been implemented and by what.
...
We have found, however, that SecPAL is a good tool for describing other policies surround the mobile ecosystem as well~\cite{hallett_specifying_2016}.
...
When considering what tools a company may use Morrow notes \emph{``particularly with the BYOD trend IT professionals don't know if anti-virus software is installed or if it's current''}~\cite{morrow_byod_2012}.
Even when devices can implement policies correctly, it is hard to ensure devices are properly configured when they are not corporately owned~\cite{tokuyoshi_security_2013}.
...
Their tool can alter what Android permissions an app is given at run time based on policies~\cite{martinelli_enhancing_2016}.
...
This allowed a company to verify the devices within their network and what servers and services they had access to.
It also describe a mechanism for providing different users with different policies.
...
Their work shows how the app-specific parts of a BYOD policy can be check and enforced using tools.
They do not look at where the checks or polices come from, however.
An \AppPAL~policy might use BYODroid to ensure that parts of a policy are enforced (as well as other tools for other parts).
One advantage of \AppPAL, however, is that it can be distributed. 
We can delegate to different marketplaces to decide if an app meets different aspects of a policy.
We can even create new stores by composing their policies and using the stores own statements about the apps.
This is very useful when using some static analysis tools which can take a long time to run (e.g.~TaintDroid~\cite{enck_taintdroid:_2014}).
...
BYOD policies have grown increasingly complex.
...
This software supports app wrapping, and supports security ``checkboxes'' to enable security controls and access control policies.
...
Our approach does not try to implement the policy by checking the apps code for programming errors:
...
They know who made what decision, and whether they made it on the basis of a policy or as a stated fact.
...
\AppPAL~inherits from SecPAL's (and Datalog's) safety condition that all variables in the head of an assertion must be referenced in the body.
...
'company' says Device canConnectToAP(AP)
  if AP isOwnedByCompany,
     ...
     AP isAP.
\end{lstlisting}
To simplify the policies we add syntactic sugar for statements giving the type of a variable (\emph{variable \emph{is}Type}).
...
The rule can be re-written as shown below hiding the typing statements, and emphasising the policy rule rather than SecPAL boiler-plate.
\begin{lstlisting}
...
It is relatively short but interesting because it contains concerns specific to healthcare scenarios, and is written as a contract the users agree to follow.
...
Finally, two simpler policies are taken from The University of Edinburgh~\cite{williamson_bring_2015} and a company specialising in emergency sirens~\cite{code3pse.org_sample_????}.
...
Apps have to be approved by three different groups\footnote{The Integrated Governance Committee, the Employee's manager, and the relevant group within the trust depending on where the employee works.} before the trust will say that an app can be installed.
\begin{policyrule}{NHS}
...
As part of selecting the apps an IT department or group may chose to use advanced instrumentation and policies~\cite{armando_enabling_2014}. 
...
As an example consider the NHS rules for finding approved apps (\autoref{ssec:byod-policies}).
...
Implementing these controls is a vital aspect of BYOD policies and all 5 of the policies we looked at had rule that described restrictions~(\autoref{tab:summary}).
...
Restricting device functionality is tricky and important, but other aspects of BYOD policies are also interesting to look at.
...
This can be a delegation to an employee's manager to authorize a decision (as in the NHS policy). 
...
The HiMSS policy is different: it  is written as a contract the user agrees to.
In the HiMMS case the user is the primary speaker.
All the policies describe multiple entities that might make assertions and be delegated to.
...
All the policies also delegate to a user (apart from HiMSS where the user is the primary speaker).
...
The HiMSS policy describes an \emph{xyz-health-system} who act similarly to an IT-department.
...
Delegations are used when a policy decision requires an input from a third-party:
...
Alongside these rules both policies also state that if an employee has personal information on the device then this will be lost as well.
...
Acknowledgements are used when there is a (usually separate) document of rules employees should be aware of and follow.
...
It can distribute electronic-documents but there is no means to track what policies a user should read and follow.
...
Once the assertion has been made it can be imported and used in making other decisions as part of the policy.
...
This gives greater flexibility to policy authors and allows them to write policies that depend on other policies rather than predefined settings and groups.
It lets us track 
...
Sometimes that is administrators and technical staff deciding what is permitted with company property, and sometimes it is the user's themselves agreeing to follow a policy.
...
Future work will look at integrating BYOD and static analysis tools into \AppPAL~so that they can be incorporated as part of the policy checks.
Whilst earlier work on SecPAL based languages~\cite{becker_secpal:_2010,moritz_y_becker_secpal:_2009} has described how assertions can be distributed there hasn't been a well defined protocol for doing so.
